...
RelArena follows the RelBench protocol for final prediction. We explicitly select
`test_timestamp` in a prediction query. The feature database remains frozen at
that cutoff. A later query timestamp emits a warning and does not expose database
rows written after `test_timestamp`.

## Setup

TabPFN-Rel is available as a standalone package on PyPI. It includes the
Relational Predictive Interface (RPI) through its `relarena-core` dependency;
the `relarena` benchmark package is only needed for the baseline comparison below.
Use Python 3.11 or 3.12.

The `api` extra runs featurization locally and uses the hosted TabPFN API for
model fitting and prediction. No local GPU is needed.

```python
!pip install -q "tabpfn-rel[api]==0.0.3"
...
print(
    ...
    f"tabpfn-rel {importlib.metadata.version('tabpfn-rel')}"
)
...
computes the label in SQL. Prediction entities and the anchor time are supplied
separately in a `PredictiveQuery`. The feature database remains frozen at
`test_timestamp`, even when a query requests a later anchor.
...
Loading the spec validates both files. A `PredictiveContext` holds the database
and training task. The calls below load each split and display its labels.
Inspect these labels before running featurization.

```python
from tabpfn_rel import PredictiveContext, PredictiveQuery, PredictiveQuerySpec, TabPFNRel
...
context.task.get_table("train", mask_input_cols=False).df.head()
```
...
`TabPFNRel(model="client")` uses hosted inference. Its `fit()` method accepts a
training context, and `predict()` accepts a query with explicit entities and a
timestamp. Use `model="local"` with the local package extra for local inference.
`n_trials=0` fits the default configuration without hyperparameter tuning.

For this historical evaluation, select sellers with observed test-window labels.
Only their IDs enter the prediction query; their outcomes are used for evaluation.
The feature database is frozen at `test_timestamp`, so rows from the forward
30-day label window are not visible to the model.

Featurization runs locally. The optional caching example below reuses these
features across fits and predictions.

```python
model = TabPFNRel(model="client")
model.fit(context, n_trials=0, seed=0)
test_labels = context.compute_test_labels()
query = PredictiveQuery(
    entities=test_labels["seller_id"].tolist(),
...
print(
    ...
    f"anchored at {spec.task.test_timestamp}"
)
preds.head()
```
...
from tabpfn_rel import PredictiveContext

CACHE_DIR = Path("./relarena-cache")
context = PredictiveContext(spec, data_version="olist-kaggle-v1")
...
The second fit can reuse features from the first fit. Its total runtime also
depends on the configurations selected by the larger tuning budget.
...
started = time.perf_counter()
preds_cached = model.predict(query)
...
prediction_anchor = spec.task.test_timestamp
print(
    f"\nChurn probabilities for {len(preds):,} sellers, "
...
`preds`, the prediction `query`, and the training `context` already exist from
the previous section.

The two baselines are supplied by `relarena`. The next cell installs that
package and calls `discover_models()` to register its models in this running
notebook. The earlier TabPFN-Rel sections do not require it.

```python
!pip install -q "relarena==0.1.1"

from relarena_core import discover_models

discover_models()

# Both baselines use the same training context and prediction cohort.
dummy = context.fit("constant-global", n_trials=0, seed=0)
preds_dummy = dummy.predict(query)

per_entity = context.fit("constant-per-entity", n_trials=0, seed=0)
preds_dummy_per_entity = per_entity.predict(query)

print(f"Held-out test sellers: {len(test_labels):,}")
```

All three models score the sellers selected by the same prediction query.
The outer join below requires predictions to match the test labels exactly,
including both seller IDs and timestamps.

```python
from sklearn.metrics import roc_auc_score


def test_roc_auc(predictions: pd.DataFrame) -> float:
...
test_scores.to_frame().round(3)
```

```console
                  Test ROC-AUC
TabPFN-Rel               0.750
Dummy per entity         0.688
Dummy                    0.500
```
