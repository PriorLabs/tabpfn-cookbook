...
![Per-policy uncertainty: the conditional cost distribution](https://raw.githubusercontent.com/PriorLabs/tabpfn-cookbook/main/visuals/insurance_claim_modeling/plot-02.png)
...
charges only ~60% of the actual claim cost. The actuarial GLMs handle the portfolio level more
...
under-pricing; **after** the single constant factor they move substantially closer to it, although
some decile-level miscalibration remains. The GLM is drawn faintly in both panels for reference, and
...
![Optional: matching the GLM on calibration](https://raw.githubusercontent.com/PriorLabs/tabpfn-cookbook/main/visuals/insurance_claim_modeling/plot-03.png)
...
![Results](https://raw.githubusercontent.com/PriorLabs/tabpfn-cookbook/main/visuals/insurance_claim_modeling/plot-04.png)
...
- **Calibration is a simple adjustment away.** Out of the box TabPFN under-prices the portfolio. A
  single factor estimated on held-out training data improves the balance from ~0.6 to ~0.96 and
  brings Tweedie deviance close to the GLMs, while provably leaving the ranking untouched.
- **The two-stage split helps TabPFN too.** Occurrence × conditional aggregate cost is better
  calibrated than a single regressor on the raw zero-inflated amount — the actuarial intuition still
  pays off.
