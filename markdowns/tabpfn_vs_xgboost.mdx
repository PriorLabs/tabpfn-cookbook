...
colab_url: https://colab.research.google.com/github/PriorLabs/tabpfn-cookbook/blob/main/notebooks/tabpfn_vs_xgboost.ipynb
---
...
results.append(
    {
        "Model": "TabPFN 3.5 (client, default)",
...
```console
   ...
   TabPFN 3.5 (client, default)  0.8246    4.20s        1.16s                                     hosted API
    XGBoost (sensible defaults)  0.8099    0.14s        0.00s                               n_estimators=100
XGBoost (CV-tuned n_estimators)  0.8018    1.07s        0.00s 5-fold CV with early stopping, best_rounds=141
```
