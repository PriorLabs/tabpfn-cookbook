...
description: Use TabPFN as a Bayesian optimization surrogate via tabpfn-extensions
  and compare it against a BoTorch GP and random search on Rosenbrock-4.
cookbookTags:
- extensions
- optimization
authors:
...
N_BO_STEPS = 25  # one objective evaluation per step
...
We run that loop `N_SEEDS = 5` times. Within a repeat, all three methods start from the **same** `N_INIT` scrambled-Sobol evaluations, so the curves can only diverge once the acquisition strategy takes over. On CPU the TabPFN loop takes a few minutes per repeat; on a GPU it is much faster.
...
```console
repeat 0: TabPFN-EI gap     85.43 | GP-LogEI (BoTorch) gap     170.1 | Random search gap      6576
repeat 1: TabPFN-EI gap     136.2 | GP-LogEI (BoTorch) gap     876.4 | Random search gap      3497
repeat 2: TabPFN-EI gap     228.4 | GP-LogEI (BoTorch) gap     333.8 | Random search gap      2376
repeat 3: TabPFN-EI gap     922.9 | GP-LogEI (BoTorch) gap     354.8 | Random search gap      1119
repeat 4: TabPFN-EI gap      1062 | GP-LogEI (BoTorch) gap     45.13 | Random search gap      3824
```
...
```console
TabPFN-EI            final gap over 5 repeats: mean       487 | median     228.4 | best     85.43
GP-LogEI (BoTorch)   final gap over 5 repeats: mean     356.1 | median     333.8 | best     45.13
Random search        final gap over 5 repeats: mean      3478 | median      3497 | best      1119
```
...
- On Rosenbrock-4 with this small budget, TabPFN-EI ends well ahead of random search on every repeat and in the same range as BoTorch's GP with LogEI: ahead on three of the five repeats, behind on two, with a lower median and a higher mean. The starting design matters a lot at 35 evaluations.
- Where TabPFN pulls clearly ahead of the GP in our larger benchmark is on multimodal functions with steep valleys (Ackley, Michalewicz) and at larger budgets, once its context has grown past a few dozen points. Use more evaluations and more repeats for a clean comparison on your own problem.
