...
- Report **repeats, not a single run**. One trace is dominated by its random init; 5 repeats with a shared init per repeat make the comparison readable, and the min–max band shows how much of the gap is real.
