...
- Extra name uses an **underscore**: `bayesian_optimization`.
- As of `tabpfn-extensions` 0.4.3 on PyPI, that extra is not published yet, so we install from the GitHub repo. Once a release includes it, switch to `pip install "tabpfn-extensions[bayesian_optimization]"`.
- The extra pulls in `botorch` for the Hartmann benchmark objective.
- Pin `tqdm` if `tabpfn-client` is also present in the environment.
...
print(f"Best found by TabPFN-BO:   {train_y.max().item():.4f}")
print(f"Optimum:                   {objective.optimal_value:.4f}")
```

```console
Best found by random search: 1.3674
Best found by TabPFN-BO:   2.3253
Optimum:                   3.3224
```

```python
fig, ax = plt.subplots(figsize=(8, 4.5))
ax.plot(range(len(bo_best)), bo_best, marker="o", label="TabPFN-BO")
ax.plot(range(1, len(random_best) + 1), random_best, marker="o", label="Random search")
ax.axhline(objective.optimal_value, linestyle="--", color="black", linewidth=1, label="Hartmann optimum")
ax.set_xlabel("Number of evaluations")
ax.set_ylabel("Best objective value so far")
ax.set_title("TabPFN Bayesian optimization vs random search")
ax.legend()
ax.grid(alpha=0.25)
plt.tight_layout()
plt.show()
...
- `tabpfn-extensions` already provides the BO loop primitive: `propose_next_point()`.
...
- On Hartmann-6, TabPFN-BO should usually beat random search under the same evaluation budget.
