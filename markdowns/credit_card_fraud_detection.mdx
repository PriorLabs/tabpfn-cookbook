...
TabPFN leads on both ranking metrics, and on the top 100 it is level with the random forest (85 versus 86
frauds) without any threshold tuning. Every ROC AUC sits above 0.92 and the top three are within three points of each other, which is
...
```console
                         Model  AUPRC ROC AUC  Frauds in top 100                                           Notes
  TabPFN 3.5 (client, default) 0.9014  0.9902                 85                                      hosted API
XGBoost (Kaggle configuration) 0.8382  0.9719                 82 max_depth=2, early stopping, best_iteration=110
    XGBoost (library defaults) 0.7813  0.9219                 79                                 XGBClassifier()
     Random forest (100 trees) 0.8734  0.9630                 86                                n_estimators=100
```
