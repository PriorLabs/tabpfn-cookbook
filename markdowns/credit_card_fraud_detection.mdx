...
```python
!pip install tabpfn-client==0.6.0 kagglehub==1.0.2 xgboost==3.4.1 scikit-learn==1.9.0 pandas==2.3.3 matplotlib==3.11.2
```
...
`kagglehub` loads public datasets straight into a dataframe without a Kaggle account. The table has no missing values, and all 30
...
df = kagglehub.dataset_load(KaggleDatasetAdapter.PANDAS, "mlg-ulb/creditcardfraud", "creditcard.csv")
...
- **Alerts to catch 90% of frauds**: sort the 56,962 test transactions by score and count how many an analyst
  team has to review before 90% of the 98 frauds are found. This is the workload behind a recall target,
  and it is where models that look alike on the first metrics come apart.
...
def alerts_for_recall(proba, target=0.9):
    """Number of highest-scored transactions to review before `target` of the frauds are found."""
    frauds_in_order = y_test.to_numpy()[np.argsort(-proba)]
    needed = int(np.ceil(target * y_test.sum()))
    return int(np.argmax(np.cumsum(frauds_in_order) >= needed)) + 1


def evaluate(name, proba, notes=""):
    alerts = alerts_for_recall(proba)
    results.append(
        {
            "Model": name,
            "AUPRC": average_precision_score(y_test, proba),
            "ROC AUC": roc_auc_score(y_test, proba),
            "Alerts for 90% recall": alerts,
            "Notes": notes,
        }
    )
    print(f"{name}: AUPRC {results[-1]['AUPRC']:.4f} | ROC AUC {results[-1]['ROC AUC']:.4f} | "
          f"{alerts:,} alerts to catch 90% of frauds")
```
...
We pin the model to TabPFN 3.5 with `create_default_for_version`, so the numbers in this notebook stay
reproducible after future releases. Use `TabPFNClassifier()` instead to always get the current default model.
...
def make_tabpfn():
    return TabPFNClassifier.create_default_for_version(ModelVersion.V3_5)
```
...
`fit` uploads the training table and `predict_proba` runs the in-context learning on Prior Labs' servers.
There are no hyperparameters to set and no class weights to pick.
...
evaluate("TabPFN 3.5 (client, default)", proba_tabpfn, "hosted API")
```

```console
TabPFN 3.5 (client, default): AUPRC 0.9014 | ROC AUC 0.9902 | 135 alerts to catch 90% of frauds
```
...
evaluate("XGBoost (Kaggle configuration)", proba_xgb_ref,
...
evaluate("XGBoost (library defaults)", proba_xgb_default, "XGBClassifier()")

rf = RandomForestClassifier(n_estimators=100, random_state=RANDOM_STATE, n_jobs=-1).fit(X_train, y_train)
proba_rf = rf.predict_proba(X_test)[:, 1]
evaluate("Random forest (100 trees)", proba_rf, "n_estimators=100")
```

```console
XGBoost (Kaggle configuration): AUPRC 0.8382 | ROC AUC 0.9719 | 647 alerts to catch 90% of frauds
XGBoost (library defaults): AUPRC 0.7813 | ROC AUC 0.9219 | 5,389 alerts to catch 90% of frauds
Random forest (100 trees): AUPRC 0.8734 | ROC AUC 0.9630 | 192 alerts to catch 90% of frauds
```
...
TabPFN leads on both ranking metrics. Every ROC AUC sits above 0.92 and the top three are within three
points of each other, which is exactly why the dataset authors ask for AUPRC: there the spread between the
same models is twelve points. The workload column makes the difference concrete: to catch 90% of the frauds,
analysts review about 135 alerts with TabPFN, roughly 190 with the random forest, and more than 600 with the
reference XGBoost, with no threshold tuning for any model.
...
```console
                         Model  AUPRC ROC AUC  Alerts for 90% recall                                           Notes
  TabPFN 3.5 (client, default) 0.9014  0.9902                    135                                      hosted API
XGBoost (Kaggle configuration) 0.8382  0.9719                    647 max_depth=2, early stopping, best_iteration=110
    XGBoost (library defaults) 0.7813  0.9219                   5389                                 XGBClassifier()
     Random forest (100 trees) 0.8734  0.9630                    192                                n_estimators=100
```
...
once. Same columns, same configurations, no tuning on the folds.
...
for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), start=1):
    ...
    row["TabPFN 3.5"] = average_precision_score(
        y_te, make_tabpfn().fit(X_tr, y_tr).predict_proba(X_te)[:, 1])
...
- **No imbalance handling needed.** TabPFN sees 394 frauds among 227,845 rows and ranks them well without
  ...
  queue, and that queue is the shortest of the four models for the same recall.
