...
Can brain protein measurements distinguish genotype, learning conditions,
and treatment in a mouse model of Down syndrome? The
[Mice Protein Expression study](https://doi.org/10.24432/C50S3Z) measures
77 proteins or protein modifications to investigate learning and the effects
of memantine, a drug studied for its ability to restore learning in these mice.
Predicting the experimental class of an unseen animal tests whether these
measurements capture biological differences that generalize across animals.

The experimental labels are already known. The research value is testing
whether protein profiles distinguish those conditions, supporting further
work to identify proteins associated with learning and treatment response.
This recipe compares **TabPFN 3, TabPFN 3.5, and XGBoost** on that prediction
task, with entire mice held out of training.
...
## The eight experimental classes

The target, `class`, combines three experimental factors: **genotype**
(control or trisomy), **conditioning** (context-shock or shock-context),
and **treatment** (saline or memantine). Context-shock (`CS`) stimulates
associative learning; shock-context (`SC`) is the comparison condition.

| Class label | Genotype | Conditioning | Treatment |
| --- | --- | --- | --- |
...
| `t-CS-s` | Trisomy | Context-shock | Saline |
| `t-CS-m` | Trisomy | Context-shock | Memantine |
| `t-SC-s` | Trisomy | Shock-context | Saline |
| `t-SC-m` | Trisomy | Shock-context | Memantine |
