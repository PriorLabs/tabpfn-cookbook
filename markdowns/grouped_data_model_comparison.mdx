...
**Grouped data** is not IID (Independent and Identically Distributed). It has an underlying structure: data groups or clusters. For example, datasets could contain multiple observations for a patient.

In this example, we will study experimental data from 72 different mice. Each mouse has 15 observations. This makes it grouped data. Two observations of the same mouse are not the same as two observations from different mice.

This also affects the evaluation of Machine Learning models. We want a model that accurately predicts data for unseen groups (here, unseen mice). We then have to test TabPFN on mice groups that were not seen in the training/context set.
...
All 15 rows from a mouse share the same target label; keeping them as separate
examples evaluates how well each protein measurement predicts that label.
...
def prepare(frame, target, split_column, train, test):
    ...
    # Learn category levels from training mice only.
...
def compare(frame, target, split_column, task_splits, name, *,
    ...
    for fold, (train, test) in enumerate(task_splits):
        ...
        for model_name, version in [
            ...
            # Score every model on the same held-out mice.
            ...
            rows.append({
                "Dataset": name, "Split": fold, "Model": model_name,
...
```python
summary = mice_results.groupby(["Dataset", "Model"], sort=False)[
    ["Log-loss", "Seconds"]
].mean()
...
```python
all_results = pd.concat([mice_results, mice_thinking], ignore_index=True)
