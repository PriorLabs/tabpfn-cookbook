...
```python
!pip install tabpfn tabpfn_extensions scikit-learn shap matplotlib
```
...
sv = explainer(X_test, max_evals=2 * len(feature_names) + 1, silent=True)
...
![Global Feature Importance](https://raw.githubusercontent.com/PriorLabs/tabpfn-cookbook/main/visuals/interpret_results/plot-01.png)
...
```console
Partial dependence for: ['Blocked vessels', 'Max heart rate', 'Cholesterol', 'ST depression']
```

![Partial Dependence](https://raw.githubusercontent.com/PriorLabs/tabpfn-cookbook/main/visuals/interpret_results/plot-02.png)
...
[Highest-risk patient] predicted risk = 100%  (base rate 53%)
    Blocked vessels    = 2                  raises risk by 12.9 points
    Thallium scan      = reversible defect  raises risk by  8.1 points
    ST depression      = 3                  raises risk by  5.6 points
    Chest pain type    = asymptomatic       raises risk by  4.9 points
    Sex                = male               raises risk by  4.4 points
```

![Explaining Individual Predictions](https://raw.githubusercontent.com/PriorLabs/tabpfn-cookbook/main/visuals/interpret_results/plot-03.png)
...
[Lowest-risk patient] predicted risk = 1%  (base rate 53%)
    Blocked vessels    = 0                  lowers risk by 10.9 points
    Chest pain type    = non-anginal        lowers risk by  9.5 points
    Sex                = female             lowers risk by  7.0 points
    Thallium scan      = normal             lowers risk by  5.1 points
    ST slope           = upsloping          lowers risk by  4.4 points
```

![Explaining Individual Predictions](https://raw.githubusercontent.com/PriorLabs/tabpfn-cookbook/main/visuals/interpret_results/plot-04.png)
...
```console
[Borderline patient] predicted risk = 50%  (base rate 53%)
    Thallium scan      = reversible defect  raises risk by 10.8 points
    Chest pain type    = non-anginal        lowers risk by 10.3 points
    ST slope           = upsloping          lowers risk by  9.5 points
    Blocked vessels    = 1                  raises risk by  7.7 points
    Sex                = male               raises risk by  5.3 points
```

![Explaining Individual Predictions](https://raw.githubusercontent.com/PriorLabs/tabpfn-cookbook/main/visuals/interpret_results/plot-05.png)
