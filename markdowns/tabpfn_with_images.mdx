...
description: Put chest X-rays in a DataFrame column next to patient fields and radiology
  labels, and let TabPFN predict from both with TabPFNWithImages from tabpfn-extensions.
icon: images
cookbookTags:
- extensions
- classification
...
The motivation comes from the [MulTaBench paper](https://arxiv.org/abs/2605.10616), which curates 40 datasets where combining tabular fields with an image or a text provably beats either modality alone. We use one of its image tasks, CheXpert: decide whether a chest X-ray shows cardiomegaly, an enlarged heart, from the scan together with the patient's sex and age, the view of the scan and the other radiology labels attached to the study. The full MulTaBench task has around 45,000 studies; we use the 5,000-row subsample published on Kaggle, so the notebook stays quick.
...
A GPU is recommended for the full subsample: embedding 5,000 X-rays and fitting TabPFN on 2,250 rows is a few minutes on a T4 and much longer on a CPU. Without one, the notebook runs on a 300-row sample so every cell finishes quickly. Set `N_ROWS = None` to use everything.
...
We fetch the CheXpert subsample from Kaggle. Each row is one chest X-ray study: the patient's sex and age, whether the view is frontal or lateral and AP or PA, thirteen other radiology labels extracted from the report (1 positive, 0 negative, -1 uncertain, blank when the report does not mention the finding), `X-Ray Image`, which is the path of the scan relative to the archive, and the target `Cardiomegaly` with the same three-way coding. Most label cells are blank; TabPFN handles missing values natively, so we leave them as they are.

```python
root = Path(kagglehub.dataset_download("chico89/multabench-chexpert-subsampled"))
...
```console
Downloading to /root/.cache/kagglehub/datasets/chico89/multabench-chexpert-subsampled/2.archive...
Extracting files...
(5000, 6)
...
1366  Female   27         Frontal    AP
                                                  ...
                                                  image
1524  /root/.cache/kagglehub/datasets/chico89/multab...
4697  /root/.cache/kagglehub/datasets/chico89/multab...
1366  /root/.cache/kagglehub/datasets/chico89/multab...
```
...
*Plain TabPFN on sex, age, view and the other radiology labels.*
...
```console
ROC AUC, picture only: 0.697
```

## Tabular + picture

Now the frame has the seventeen tabular columns followed by `image`, so the image column sits at position `len(tabular)`. Everything else is unchanged.
...
ax.barh(list(results), list(results.values()), color=["#9aa5b1", "#9aa5b1", "#2f6fed"])
ax.axvline(0.5, linestyle="--", color="grey", linewidth=1)
ax.text(0.5, -0.55, "random guess", fontsize=8, color="grey", ha="center")
ax.set_xlim(0.4, 1.0)
...
tabular only         0.621
picture only         0.697
tabular + picture    0.702
```

![Tabular + picture](https://raw.githubusercontent.com/PriorLabs/tabpfn-cookbook/main/visuals/tabpfn_with_images/plot-02.png)

Whether the tabular fields or the X-ray come out ahead on their own depends on the split and on how many rows you use, but the point of the exercise is the third bar: the scan carries signal about an enlarged heart that the patient's fields and the other labels do not have, and the tabular fields carry signal the scan does not have, so the combined model should be the best of the three. That is not a lucky pick of dataset. MulTaBench only includes tasks where "both beats either" held across several different learners, so this pattern is what the benchmark was built to isolate. For more tasks, the [paper](https://arxiv.org/abs/2605.10616) and the [MulTaBench datasets on Kaggle](https://www.kaggle.com/chico89/datasets) have 20 image-tabular tasks, all downloadable with `kagglehub` like the one used here.
