...
A GPU is recommended for the full subsample: embedding 5000 X-rays and fitting TabPFN on 3750 rows is a few minutes on a T4 and much longer on a CPU. Without one, the notebook runs on a 300-row sample so every cell finishes quickly. Set `N_ROWS = None` to use everything.
